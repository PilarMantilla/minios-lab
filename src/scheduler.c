...
int scheduler_create_process(const char *path, const char *arg) {
    ...
    if (pid > 0 && platform_uses_ptrace()) {
        ...
        if (!WIFSTOPPED(status)) {
            fprintf(stderr, "Error: child did not stop as expected\n");
            kill(pid, SIGKILL);
            return -1;
        }
    }
...
void scheduler_start(int slice_ms) {
    ...
    //         Si la cola quedó vacía, imprimir error y retornar.
    int idx = rq_dequeue();
...
void scheduler_tick(int signum) {
    ...
    double elapsed =
        timespec_diff_ms(now, current->last_started);
